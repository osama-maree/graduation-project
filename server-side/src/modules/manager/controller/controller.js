...
export const signup = async (req, res, next) => {
  try {
    ...
    if (employee) {
      next(new Error("this admin is exit", { cause: 409 }));
    } else {
...
<body style="background-color: #e9ecef;">
  ...
  <!-- end body -->
      ...
      res
        .status(201)
        .json({ message: "success,verify gmail", user: savedUser });
    }
  } catch (err) {
...
export const getEmp = async (req, res, next) => {
  try {
    const employee = await employeeModel.find({ role: "employee" });
  ...
  try {
    ...
    if (type === "sale") {
      trans = await saleModel
        ...
        .select("createdAt updatedAt MoveState id userId");
    } else if (type === "mort") {
      trans = await mortgagaModel
        .find({
          employeeId: id,
        })
        .populate({
...
export const getEmpforAdmin = async (req, res, next) => {
  try {
...
export const giveVacation = async (req, res, next) => {
  try {
    const { _id, type } = req.body;

    if (type === "type1") {
      await employeeModel.findByIdAndUpdate(_id, { freez: false });
    } else if (type === "type2") {
      await employeeModel.findByIdAndUpdate(_id, { freez: true });
    }
